...
class MultiScaleDeformableAttention3D(BaseModule):
    ...
    def forward(self,
        ...
        if query_pos is not None:
            query = query + query_pos #将查询 query 与位置编码 query_pos 相加，增强查询的空间信息。
        
        if not self.batch_first:  #维度调整
            # change to (bs, num_query ,embed_dims)
            query = query.permute(1, 0, 2)  #torch.Size([1, 600, 256])
            value = value.permute(1, 0, 2)  #torch.Size([1, 32400, 256])
        ...
        if key_padding_mask is not None:
            value = value.masked_fill(key_padding_mask[..., None], 0.0)  #如果有 key_padding_mask，将无效区域的值置零
        value = value.view(bs, num_value, self.num_heads, -1) #将 value 重塑为多头形式  #torch.Size([1, 40000, 8, 32])
        
        sampling_offsets = self.sampling_offsets(query).view(
            bs, num_query, self.num_heads, self.num_levels, self.num_points, 3) #采样偏移
        ...
        offset_normalizer = torch.stack(   #根据 spatial_shapes（(num_levels, 3)）归一化偏移量，确保偏移在空间尺度上合理。
            [spatial_shapes[..., 2], 
             spatial_shapes[..., 1], 
             spatial_shapes[..., 0]], -1)
        # exit()
        ...
        sampling_locations = reference_points[:, :, None, :, None, :] \
            + sampling_offsets \
            / offset_normalizer[None, None, None, :, None, :]   #将参考点 reference_points（(bs, num_query, num_levels, 3)）与归一化的偏移量相加，得到最终采样位置
        
        output = multi_scale_deformable_attn_pytorch(
            value, spatial_shapes, sampling_locations, attention_weights)  #基于采样位置和注意力权重计算输出。
        output = self.output_proj(output)  #torch.Size([1, 32400, 256])

        if not self.batch_first:
            # (num_query, bs ,embed_dims)
            output = output.permute(1, 0, 2)

        return self.dropout(output) + identity  #残差连接（+ identity）
