...
# from mmdet3d.models.backbones.resnet_cotr import ResNet_COTR
...
class COTR(BEVStereo4D):
    ...
    def bev_encoder(self, x):
        ...
        #     x = x[0]
        return x, feats   #x的torch.Size([1, 32, 16, 50, 50])
    ...
    def extract_img_feat(self,
        ...
        bda, curr2adjsensor = self.prepare_inputs(img_inputs, stereo=False)  #0809 由True改成了False
        ...
        num_frames = len(imgs) #0810 # 用 imgs 的长度代替 self.num_frame，保证单帧时循环没越界
        # for fid in range(self.num_frame-1, -1, -1): #原先的
        for fid in range(num_frames-1, -1, -1):  #0810
            ...
            key_frame = fid == 0
            # extra_ref_frame = fid == self.num_frame-self.extra_ref_frames #原先的
            if num_frames == 1:  #0810 保证单帧
                extra_ref_frame = False  #0810 保证单帧
            else:                        #0810 保证单帧
                extra_ref_frame = fid == num_frames - self.extra_ref_frames  #0810 保证单帧
            if key_frame or self.with_prev:
                ...
                if key_frame:
                    ...
                    with torch.no_grad():
                        bev_feat, depth, feat_curr_iv, img_f = \
                            self.prepare_bev_feat(*inputs_curr)
                # if not extra_ref_frame:  #原先的
                #     bev_feat_list.append(bev_feat)  #原先的
                
                if (not extra_ref_frame) or (self.num_frame == 1): #0809
                    bev_feat_list.append(bev_feat) #0809
        ...
        x, feats = self.bev_encoder(bev_feat)  #x是torch.Size([1, 32, 16, 50, 50])
        return [x, feats, img_feature_key_frame], depth_key_frame, img_metas, cam_params

    def extract_feat(self,  img_inputs, img_metas, **kwargs):  #没用点云，只是用了图像
        """Extract features from images and points."""
        img_feats, depth, img_metas, cam_params = self.extract_img_feat(img_inputs, img_metas, **kwargs)
        return (img_feats,  depth, img_metas, cam_params)  #img_feats是0 1 2 ，1的shape是torch.Size([1, 32, 16, 50, 50])
    ...
    def forward_pts_train(self,
        ...
        outs = self.pts_bbox_head(pts_feats, depth, img_metas, 
                                  cam_params, **kwargs)        #pts_bbox_head是COTRHead
    ...
    def forward_train(self,
        ...
        """
        img_feats,  depth, img_metas, cam_params = self.extract_feat(
        ...
        return img_feats, depth, img_metas, cam_params
...
class COTR_Group(COTR):
    ...
    def forward_train(self,
        ...
        """
        img_feats,  depth, img_metas, cam_params = self.extract_feat(
            img_inputs=img_inputs, img_metas=img_metas, **kwargs)   #调用提取图像函数
        voxel_semantics = kwargs['voxel_semantics']
        mask_camera = kwargs['mask_camera']
        mask_lidar = kwargs['mask_lidar']
        
        gt_classes, sem_mask = self.generate_group(voxel_semantics)  #调用生成分组标签​函数

        # losses = dict()
        fullres_occ = self.forward_pts_train(img_feats, depth, voxel_semantics=voxel_semantics,
                                          gt_classes=gt_classes, sem_mask=sem_mask,
                                          mask_camera=mask_camera, mask_lidar=mask_lidar,
                                          img_metas=img_metas, cam_params=cam_params)
        # losses.update(loss_occ)   # 占据预测损失
        # gt_depth = kwargs['gt_depth']
        # loss_depth = self.img_view_transformer.get_depth_loss(gt_depth, depth)  #深度估计损失
        # # losses['loss_depth'] = loss_depth
        # # return losses
        return fullres_occ
