...
class BEVDet(CenterPoint):
    ...
    def image_encoder(self, img_inputs, stereo=False):
        imgs = img_inputs
    ...
    def extract_img_feat(self, img_inputs, img_metas, **kwargs):
        ...
        img = self.prepare_inputs(img_inputs)
    ...
    def forward_test(self,
        ...
        """
        # # ----- 0818 xy 添加封装，解决 len(img_inputs) != len(img_metas) -----
        # if img_inputs is not None and img_metas is not None:
        #     if len(img_inputs) != len(img_metas):
...
class BEVDet4D(BEVDet):
    ...
    def __init__(self,
        ...
        self.num_frame = num_adj + 1    # 时序，num_adj是1，这里num_frame还是2，在BEVStereo4D类的init变成了3
    ...
    def prepare_inputs(self, inputs, stereo=False):
        ...
        # # --- #0810 单帧模式 ---
        if N <= 6:  # 这里6表示单帧6个相机
            # self.num_frame = 1  # 保证循环不会越界
            imgs = [inputs[0]]  # 保持list结构
        ...
        # --- 多帧模式（原逻辑） ---
        N = N // self.num_frame  #原先有时序的时候是18/3=6。如果不加单帧模式的逻辑，self.num_frame是3，送进来的N不是18，是6，就会变成6/3=2，导致只有2个摄像头。
        ...
        global2keyego = torch.inverse(keyego2global.double())
        sensor2keyegos = \
            global2keyego @ ego2globals.double() @ sensor2egos.double()  #这是为了时序帧对齐的，把所有非关键帧的相机位姿转换到关键帧 ego 坐标系下
        sensor2keyegos = sensor2keyegos.float()
...
class BEVStereo4D(BEVDepth4D):
    def __init__(self, **kwargs):
        ...
        self.num_frame += self.extra_ref_frames       # 时序#这里self.num_frame变成了3
    ...
    def extract_img_feat(self,
        ...
        for fid in range(self.num_frame-1, -1, -1):
            ...
            if key_frame or self.with_prev:
                ...
                if curr2adjsensor is not None:  #0809
                    curr2adjsensor_item = curr2adjsensor[fid]  #0809
