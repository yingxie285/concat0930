...
class BEVFusion(Base3DFusionModel):
    def __init__(
        ...
        **kwargs,
            ...
            self.encoders["camera"] = nn.ModuleDict(
                {
                    ...
                    "bev_encoder_backbone":build_backbone(encoders["camera"]["bev_encoder_backbone"]),
                    "bev_encoder_neck": build_neck(encoders["camera"]["bev_encoder_neck"])
                }
            )
        ...
        self.init_weights()  #原先的，用了resnet的初始权重文件，URL方法获取的权重

    def init_weights(self) -> None:  #原先的，用了resnet的初始权重文件，URL方法获取的权重
    ...
    def extract_camera_features(
        ...
        **kwargs,
        ...
        x = x.view(B, int(BN / B), C, H, W)  #shape:torch.Size([4, 6, 256, 32, 88])

        x = self.encoders["camera"]["vtransform"](  #原先的
            x,
            points,
            camera2ego,
            lidar2ego,
        ...
        return x  #shape:torch.Size([4, 1280, 180, 180])  图像BEV特征
    ...
    def extract_lidar_features(self, x) -> torch.Tensor:
        ...
        return x  #torch.Size([4, 256, 180, 180])  点云BEV
