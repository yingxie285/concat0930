...
class BEVTransform(BaseModule):
    ...
    def forward(self,
        ...
        Returns:
        """
        B, N, C, H, W = x.shape  ##shape:torch.Size([4, 6, 256, 32, 88])
        x = x.view(B * N, C, H, W)
        ...
        sampling_feats = einops.rearrange(sampling_feats,
                                          ...
                                          b=bs)
        feats_volume = x.new_zeros([bs, num_voxels, C])  #torch.Size([4, 324000, 128])
        for j in range(bs):
            indexes_per_batch = indexes[j]
        ...
        feats_volume = feats_volume / count[..., None]  #torch.Size([4, 324000, 128])
        ...
        return feats_volume  #torch.Size([4, 1280, 180, 180])
