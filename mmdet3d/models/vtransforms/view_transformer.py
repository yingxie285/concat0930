...
class LSSViewTransformer(BaseModule):
    ...
    def create_frustum(self, depth_cfg, input_size, downsample):
        ...
        d = torch.arange(*depth_cfg, dtype=torch.float)\
            .view(-1, 1, 1).expand(-1, H_feat, W_feat) #shape:torch.Size([88, 16, 44])
        self.D = d.shape[0]
        if self.sid:
        ...
        x = torch.linspace(0, W_in - 1, W_feat,  dtype=torch.float)\
            .view(1, 1, W_feat).expand(self.D, H_feat, W_feat) #shape:torch.Size([88, 16, 44])
        y = torch.linspace(0, H_in - 1, H_feat,  dtype=torch.float)\
            .view(1, H_feat, 1).expand(self.D, H_feat, W_feat) #shape:torch.Size([88, 16, 44])
...
class DepthNet(nn.Module):
    ...
    def forward(self, x, mlp_input, stereo_metas=None):
            ...
            if stereo_metas['cv_feat_list'][0] is None:
                ...
                cost_volumn = \
                    torch.zeros((BN, self.depth_channels,
                                 int(H*scale_factor),
                                 int(W*scale_factor))).to(x)  #原先的,stereo_metas['cv_feat_list'][0]是空，会创建全0的成本体
            else:
                with torch.no_grad():
        ...
        return torch.cat([depth, context], dim=1)  #原先的
...
class LSSViewTransformerBEVDepth(LSSViewTransformer):
    ...
    def forward(self, input, stereo_metas=None):
        (x, rots, trans, intrins, post_rots, post_trans, bda,
         mlp_input) = input[:8]  #mlp_input是torch.Size([1, 2, 27])

        B, N, C, H, W = x.shape  #带时序是torch.Size([1, 18, 256, 16, 44]) 去掉时序是torch.Size([1, 6, 256, 16, 44])
        x = x.view(B * N, C, H, W)  #torch.Size([6, 256, 16, 44])
        x = self.depth_net(x, mlp_input, stereo_metas)  #torch.Size([8, 120, 16, 44]) 原先的
        depth_digit = x[:, :self.D, ...]
        tran_feat = x[:, self.D:self.D + self.out_channels, ...]
        depth = depth_digit.softmax(dim=1)
        bev_feat, depth = self.view_transform(input, depth, tran_feat)  #bev_feat是shape:torch.Size([1, 32, 16, 200, 200])  depth是shape:torch.Size([6, 88, 16, 44])
        return bev_feat, depth
...
# 注意: 这个修改假设 LSSViewTransformerBEVStereo 的原始代码已存在。
# 我们创建一个新的类 ModifiedLSSViewTransformerBEVStereo，继承自 LSSViewTransformerBEVStereo。
# 在这个类中，我们覆盖 forward 方法，并集成 BEVTransformV2 的 point_sampling 和采样逻辑。
# 关键变化:
# 1. 在 __init__ 中，添加 ref_3d 的生成（类似于 BEVTransformV2 中的 get_reference_points）。
# 2. 添加 transfer_conv 如果需要（如果 in_channels != out_channels）。
# 3. 在 forward 中，运行原始的 depth_net 以获取 tran_feat（上下文特征）和 depth（用于损失）。
# 4. 然后，使用 point_sampling 生成 reference_points_img 和 volume_mask。
# 5. 使用 F.grid_sample 从 tran_feat (重排列为 B C N H W) 上采样特征。
# 6. 聚合到体素特征 feats_volume，并 rearrange 到 BEV 形状。
# 7. 如果有 down_sample，应用它（从 BEVTransformV2 复制，如果需要）。
# 8. 参数映射:
#    - camera2sensor: 假设为 camera2lidar，如果 top_type='lidar'。这里我们构建为 4x4 矩阵（从 rots 和 trans）。
#    - cam2imgs: intrins。
#    - post_rots, post_trans: 直接使用。
#    - bda: LSS 中的 bda 是 3x3，我们扩展为 4x4，假设平移为 0。
# 9. 假设 get_reference_points 函数定义如下（你可以根据需要实现）。


def get_reference_points(x_min, x_max, y_min, y_max, z_min, z_max, xs, ys, zs):
    # 生成 BEV 网格的参考点（每个体素的中心）
    dx = (x_max - x_min) / xs
    dy = (y_max - y_min) / ys
    dz = (z_max - z_min) / zs
    x = torch.linspace(x_min + dx / 2, x_max - dx / 2, xs)
    y = torch.linspace(y_min + dy / 2, y_max - dy / 2, ys)
    z = torch.linspace(z_min + dz / 2, z_max - dz / 2, zs)
    x, y, z = torch.meshgrid(x, y, z, indexing='ij')
    ref_3d = torch.stack((x, y, z), -1).reshape(-1, 3)
    return ref_3d

@NECKS.register_module()
class ModifiedLSSViewTransformerBEVStereo(LSSViewTransformerBEVStereo):
    def __init__(self, grid_config, input_size, in_channels, out_channels, top_type='lidar',
                 down_sample=False, down_sample_scale=2, down_sample_channels=[128*10, 64*10, 32*10, 16*10],
                 **kwargs):
        super().__init__(
        ...
        z_cfg = grid_config['z']
        self.volume_size = [
            int((x_cfg[1] - x_cfg[0]) / x_cfg[2]),
            int((y_cfg[1] - y_cfg[0]) / y_cfg[2]),
            int((z_cfg[1] - z_cfg[0]) / z_cfg[2])
        ]
        ref_3d = get_reference_points(
            x_cfg[0], x_cfg[1], y_cfg[0], y_cfg[1], z_cfg[0], z_cfg[1],
        ...
        if down_sample:
            ...
            if down_sample_scale == 2:
                self.down_sample = nn.Sequential(
                    ...
                    nn.ReLU(True),
                    nn.Conv2d(down_sample_channels[1], down_sample_channels[2], 3, stride=down_sample_scale, padding=1, bias=False),
    ...
    @force_fp32()
    def forward(self, input, stereo_metas=None):
        (imgs, sensor2egos, ego2globals, intrins, post_rots, post_trans, bda_rot, mlp_input) = input[:8]
        ...
        tran_feat = einops.rearrange(tran_feat, '(B N) C H W -> B C N H W', B=B)
        
        # 检查 sensor2egos 和 ego2globals 的形状
        if sensor2egos.shape != ego2globals.shape or sensor2egos.dim() != 4 or sensor2egos.shape[2:] != (4, 4):
            raise ValueError(f"Expected sensor2egos and ego2globals to have shape [B, N, 4, 4], "
                             f"but got sensor2egos {sensor2egos.shape} and ego2globals {ego2globals.shape}")

        # 根据 top_type 设置 camera2sensor
        if self.top_type == 'lidar':
            camera2sensor = sensor2egos  # [B, N, 4, 4]
        elif self.top_type == 'ego':
            # 相机到全局坐标系：sensor2egos @ ego2globals
            camera2sensor = torch.matmul(sensor2egos, ego2globals)  # [B, N, 4, 4]
        else:
            raise NotImplementedError(f"Unsupported top_type: {self.top_type}")
        
        # 构建 bda_4x4
        zeros_col = torch.zeros(B, 3, 1, device=bda_rot.device)
        bda_4x4 = torch.cat((bda_rot, zeros_col), dim=-1)
        zeros_row = torch.zeros(B, 1, 4, device=bda_rot.device)
        zeros_row[..., 3] = 1.0
        bda_4x4 = torch.cat((bda_4x4, zeros_row), dim=-2)
        
        reference_points_img, volume_mask = self.point_sampling(
            camera2sensor, intrins, post_rots, post_trans, bda_4x4
        )
        ...
        sampling_feats = einops.rearrange(sampling_feats, 'b c max_len 1 1 -> b max_len c', b=bs)
        feats_volume = tran_feat.new_zeros([bs, num_points, self.out_channels])
        for j in range(bs):
        ...
        # if self.collapse_z:
        #     feats_volume = torch.cat(feats_volume.unbind(dim=1), 1)
        
        return feats_volume, depth
...
# 使用方法:
# 在配置文件中，将 type 改为 'ModifiedLSSViewTransformerBEVStereo'
# 确保 grid_config 和 input_size 等一致。
# 如果 top_type='ego'，需要处理 camera_ego2global（从 kwargs 获取）。
# 测试时，确保所有参数形状匹配。