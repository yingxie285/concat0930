...
class FPN_LSS(BaseModule):
    ...
    def forward(self, feats):
        ...
        Returns:
            x: (B, C_out, 2*H, 2*W)
        """
        x2, x1 = feats[self.input_feature_index[0]], feats[self.input_feature_index[1]]  #input_feature_index是0和2
        if self.lateral:
            x2 = self.lateral_conv(x2)
        x1 = self.up(x1)    # (B, C3, H, W)
        x1 = torch.cat([x2, x1], dim=1)     # (B, C1+C3, H, W) torch.Size([4, 640, 180, 180])
        x = self.conv(x1)   # (B, C', H, W)  torch.Size([4, 512, 180, 180])
        if self.extra_upsample:
            x = self.up2(x)     # (B, C_out, 2*H, 2*W)
        return x  #torch.Size([4, 512, 180, 180])
