...
class CustomResNet(BaseModule):  # TODO: [yz] check param init
    def __init__(
            self,
            numC_input,
            num_layer=[2, 2, 2],  #指定每个阶段（stage）的残差块数量，默认 [2, 2, 2]，表示 3 个阶段，每阶段有 2 个残差块。
            num_channels=None,
            stride=[2, 2, 2],
        ...
        self.layers = nn.Sequential(*layers)  #解包，组成更大的容器（Sequential）卷积模块
    ...
    def forward(self, x):
        ...
        """
        feats = []
        x_tmp = x  #torch.Size([4, 512, 180, 180])
        for lid, layer in enumerate(self.layers):
            if self.with_cp:
                x_tmp = checkpoint.checkpoint(layer, x_tmp)
            else:
                x_tmp = layer(x_tmp) #第0次torch.Size([4, 128, 180, 180]) 第1次torch.Size([4, 256, 180, 180]) 第2次torch.Size([4, 512, 90, 90])
            if lid in self.backbone_output_ids:  #start:0  step:1  stop:3
                feats.append(x_tmp)
        return feats   #0是torch.Size([4, 128, 180, 180])  1是torch.Size([4, 256, 180, 180])  2是torch.Size([4, 512, 90, 90])
