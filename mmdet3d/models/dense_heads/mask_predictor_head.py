...
class MaskPredictorHead_Group(BaseModule):
    ...
    def forward(self, occ_feature, query):
        ...
        if self.use_checkpoint:
            ...
            occ_feature = self.decoder(occ_feature)  #将 occ_feature 的每个体素的特征从 in_dims 投影到 out_dims（通过 hidden_dims） torch.Size([1, 200, 200, 16, 256])
            # logits = self.classifier(fused)
        # logits = logits.permute(0, 4, 1, 2, 3) # [bs, c, w, h, z]
        occ_feature = occ_feature.permute(0, 4, 1, 2, 3) # 投影后的体视网格特征[bs, c, w, h, z]  torch.Size([1, 256, 200, 200, 16])

        query = query.permute(0, 2, 1, 3)  #torch.Size([1, 1, 600, 256])
        num_query = query.shape[-2]  #600
        num_query_per_group = num_query // self.group_detr  #600/6=100
        output_classes = []
        if self.mask_classification:
            if not self.training:
                ...
                for group_index in range(self.group_detr): #self.group_detr是6
                    group_query_start = group_index * num_query_per_group  #0
                    group_query_end = (group_index+1) * num_query_per_group  #100
                    group_query = query[:, :, group_query_start:group_query_end, :]  #torch.Size([1, 1, 100, 256])
                    output_class = self.class_embed[group_index](group_query)  #每组使用独立的 class_embed[group_index]生成分类预测 torch.Size([1, 1, 100, 18])，self.class_embed：一个线性层（或 group_detr 个线性层的列表），将查询特征（embed_dims=256）映射到类别数
                    output_classes.append(output_class) #分类 logits，表示每个查询对应的类别概率（包括一个“无物体”类别）
            # for group_index in range(self.group_detr):
        ...
        # [layer, bs, N, c]
        mask_embed = self.mask_embed(query)   #shape:torch.Size([1, 1, 600, 256])
        outputs_seg_masks = torch.einsum("lbnc,bcwhz->lbnwhz", mask_embed, occ_feature)  #occ_feature是torch.Size([1, 256, 200, 200, 16])
