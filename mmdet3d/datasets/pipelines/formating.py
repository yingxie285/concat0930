...
@PIPELINES.register_module()
class WrapImgInputs:
    """Wrap img_inputs into a single-element list.
    
    This transform takes the original result['img_inputs'] (e.g., a tuple/list of length 7)
    and wraps it into a list where result['img_inputs'] = [original_img_inputs].
    ...
    def __call__(self, results):
        """Transform function to wrap img_inputs.
        
        Args:
            results (dict): Result dict containing the data to transform.
        
        Returns:
            dict: Updated result dict with img_inputs wrapped as a single-element list.
        """
...
class Collect3D:
    def __init__(
        ...
        self.meta_lis_keys_yh = meta_lis_keys_yh
    ...
    def __call__(self, results):
        ...
        metas_yh = {}
        for key in self.meta_lis_keys_yh:
            if key in results:
                metas_yh[key] = results[key]
        
            # 重命名token和lidar_path
        if "token" in metas_yh:
            metas_yh["sample_idx"] = metas_yh.pop("token")
        if "lidar_path" in metas_yh:
            metas_yh["pts_filename"] = metas_yh.pop("lidar_path")

        data["img_metas"] = DC(metas_yh, cpu_only=True)

        # try:
        #     def safe_serialize(obj):
        #         if isinstance(obj, DC):
        #             return f"DataContainer({str(obj)})"
        #         try:
        ...
        #     serializable_data = {k: safe_serialize(v) for k, v in data.items()}
        #     with open("collect3d_output_new.txt", "a") as f:
