...
class NuScenesDatasetOccupancy(NuScenesDataset):
    ...
    def get_data_info(self, index):
        ...
        # Extend sweeps with camera data
        for sweep in input_dict.get('sweeps', [])[:2]:   #yh
            sweep['cams'] = {
                f'CAM_{i}': {
                    'data_path': input_dict['image_paths'][i],  # Placeholder; need actual sweep camera paths
                    'camera_intrinsics': input_dict['camera_intrinsics'][i][:3, :3],
                    'sensor2ego_rotation': input_dict['camera2ego'][i][:3, :3].tolist(),
                    'sensor2ego_translation': input_dict['camera2ego'][i][:3, 3].tolist(),
                    'ego2global_rotation': sweep.get('ego2global_rotation', input_dict['ego2global'][:3, :3]),
                    'ego2global_translation': sweep.get('ego2global_translation', input_dict['ego2global'][:3, 3])
                } for i in range(len(input_dict['image_paths']))
            }
        return input_dict
...
# @DATASETS.register_module()
# class NuScenesDatasetOccpancy_COTR(NuScenesDataset_COTR):
#     def get_data_info(self, index):
#         """Get data info according to the given index.
...
#         """
#         input_dict = super(NuScenesDatasetOccpancy_COTR, self).get_data_info(index)
#         # standard protocol modified from SECOND.Pytorch
