...
class NuScenesDataset(Custom3DDataset):
    ...
    #     "barrier",
    # )
    # TODO: [yz] correct CLASSES:  #这是DAOcc默认的类别的顺序，和default.yaml是一样的
    # - car
    # - truck
    # - construction_vehicle
    # - bus
...
# @DATASETS.register_module()
# class NuScenesDataset_COTR(Custom3DDataset_COTR):
#     r"""NuScenes Dataset.
...
#                  eval_version='detection_cvpr_2019',
#                  use_valid_flag=False,
#                  img_info_prototype='mmcv',  #COTR独有
#                  multi_adj_frame_id_cfg=None,  #COTR独有
#                  ego_cam='CAM_FRONT',     #COTR独有
#                  stereo=False):           #COTR独有
#         self.load_interval = load_interval
#         self.use_valid_flag = use_valid_flag
#         super().__init__(
#             data_root=data_root,  #一样的，都是data/nuscenes/
#             ann_file=ann_file,
#             pipeline=pipeline,
#             classes=classes,
#             modality=modality,
...
#         self.img_info_prototype = img_info_prototype    #COTR独有
#         self.multi_adj_frame_id_cfg = multi_adj_frame_id_cfg  #COTR独有
#         self.ego_cam = ego_cam   #COTR独有 
#         self.stereo = stereo     #COTR独有

#     def get_cat_ids(self, idx):  #和DAOcc一样
#         """Get category distribution of single scene.
...
#     def load_annotations(self, ann_file):  #和DAOcc一样
#         """Load annotations from ann_file.
...
#     def get_data_info(self, index):  #有不同
...
#         input_dict = dict(    #这里和DAOcc不同
#             sample_idx=info['token'],
#             pts_filename=info['lidar_path'],
#             sweeps=info['sweeps'],
#             timestamp=info['timestamp'] / 1e6,  #COTR是转化为秒
#         )
...
#     def get_adj_info(self, info, index):  #COTR独有
#         info_adj_list = []
...
#     def get_ann_info(self, index):  #和DAOcc一样
#         """Get annotation info according to the given index.
...
#     def _format_bbox(self, results, jsonfile_prefix=None):  #有不同
#         """Convert the results to the standard format.
...
#                 nusc_box = NuScenesBox(center, wlh, quat, velocity=box_vel)  #组成nusc_box
#                 nusc_box.rotate(rot)
...
#     def _evaluate_single(self,                 #和DAOcc类似，返回的名字不同
#                          result_path,
#                          logger=None,
#                          metric='bbox',
#                          result_name='pts_bbox'):
...
#     def format_results(self, results, jsonfile_prefix=None):   #有不同
#         """Format the results to json (standard format for COCO evaluation).
...
#         if not ('pts_bbox' in results[0] or 'img_bbox' in results[0]):  #不同
#             result_files = self._format_bbox(results, jsonfile_prefix)
#         else:
...
#     def _build_default_pipeline(self):  #COTR独有
...
#     def show(self, results, out_dir, show=False, pipeline=None):  ##COTR独有
...
#     # our LiDAR coordinate system -> nuScenes box coordinate system（wlh）
#     nus_box_dims = box_dims[:, [1, 0, 2]]  #COTR多这一行
...
# def lidar_nusc_box_to_global(info,   #和DAOcc一样
#                              boxes,
#                              classes,
#                              eval_configs,
#                              eval_version='detection_cvpr_2019'):
